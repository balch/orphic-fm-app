...
/**
 * Float-native audio input port for the Oboe backend.
 ...
 * thread at any time. They publish an immutable snapshot array via @Volatile,
...
class OboeAudioInput(
    ...
    private val sourcesList = mutableListOf<OboeAudioOutput>()
    // Immutable snapshot — published by wiring thread, read by audio thread
    @Volatile private var sourcesSnapshot: Array<OboeAudioOutput> = emptyArray()
    ...
    internal fun connectFrom(output: OboeAudioOutput) {
        synchronized(sourcesList) {
    ...
    fun prepare(numFrames: Int) {
        ...
        val snap = sourcesSnapshot // Single volatile read
        when (snap.size) {
            0 -> {
                if (smoothed) {
            ...
            1 -> {
                ...
                if (src.size >= numFrames) {
                    System.arraycopy(src, 0, buffer, 0, numFrames)
                }
            }
            else -> {
                ...
                if (first.size >= numFrames) {
                    System.arraycopy(first, 0, buffer, 0, numFrames)
                } else {
    ...
    companion object {
        /** One-pole coefficient for ~5ms smoothing. */
        private val SMOOTH_COEFF = 1f - exp(-1f / (0.005f * DSP_SAMPLE_RATE))
    }
    ...
    internal fun getSources(): List<OboeAudioOutput> = sourcesList
}

/**
 * Float-native audio output port for the Oboe backend.
 */
class OboeAudioOutput(private val name: String = "") : AudioOutput {
    private var buffer = FloatArray(0)
