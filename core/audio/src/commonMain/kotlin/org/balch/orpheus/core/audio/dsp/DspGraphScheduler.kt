 ...
 * Called from the platform audio thread (e.g., Oboe on Android, ScriptProcessorNode on WASM).
 */
class DspGraphScheduler {
    private val units = mutableListOf<DspProcessable>()
    private val unitSet = HashSet<DspProcessable>() // dedup guard
    private var maxFrames = 0

    val masterLeft = DspAudioInput("MasterLeft")
    val masterRight = DspAudioInput("MasterRight")

    val unitCount: Int get() = units.size

    fun addUnit(unit: DspProcessable) {
        if (unitSet.add(unit)) {
            units.add(unit)
        }
    }
    ...
    fun sortTopologically() {
        ...
        val outputOwner = HashMap<DspAudioOutput, Int>(n * 2)
        val unitIndex = HashMap<DspProcessable, Int>(n * 2)
        for (i in 0 until n) {
            val unit = units[i]
            unitIndex[unit] = i
        ...
        val sorted = ArrayList<DspProcessable>(n)
        for (s in sccOrder) {
    ...
    companion object {
        private val log = logging("DspGraphScheduler")
