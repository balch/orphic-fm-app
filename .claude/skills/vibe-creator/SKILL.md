...
Convention (not enforced): 0=kick, 1=snare, 2=hat, 3=bass, 4=keys/lead, 5-6=texture/FX, 7=wildcard. Any track can use any engine. The index does carry engine behaviour, though: tracks 5-7 ride the FX fader and are volume-notched to 0.05 across energy 0.45..0.55 (`texture_energy_curve`), track 7's note-ons are probability-gated (silent at default macros), and tracks 3-4 get a mid-energy boost instead. A `Melodic` track with a `lickMode` other than `None` and any `Chordal` track skip the notch, so a riff double, harmony line or comping organ on 5-6 is heard at mid energy; a `Percussive` hit or a lick-less melodic pad or drone there is not.
...
- **`lpgMode`** (default `ENGINE_DEFAULT`): Vactrol LPG mode — `BYPASS` (raw), `SUSTAINED` (gate-following), `PLUCK` (asymmetric bloom), `PLUCK_REPEAT` (PLUCK that re-picks a held note on every hold step: a 1-beat lick note becomes four 16th picks, the double-picked riff), `PLUCK_REPEAT_8TH` / `PLUCK_REPEAT_8TH_OFF` / `PLUCK_REPEAT_TRIPLET` / `PLUCK_REPEAT_16TH_OFF` (the same re-pick on the beat and the "&", on the "&" only, on 8th-note triplets, or on the "e" and "a" only; the grid follows the beat and a note's first pick always plays), or `ENGINE_DEFAULT` (consult per-engine table). Set explicitly per voice when EDM/Space want different envelope behavior (e.g. `PLUCK` for a WSH bass on EDM, `BYPASS` for a STR drone on Space).
