...
- **`lpgMode`** (default `ENGINE_DEFAULT`): Vactrol LPG mode — `BYPASS` (raw), `SUSTAINED` (gate-following), `PLUCK` (asymmetric bloom), `PLUCK_REPEAT` (PLUCK that re-picks a held note on every hold step: a 1-beat lick note becomes four 16th picks, the double-picked riff), `PLUCK_REPEAT_8TH` / `PLUCK_REPEAT_8TH_OFF` / `PLUCK_REPEAT_TRIPLET` (the same re-pick on the beat and the "&", on the "&" only, or on 8th-note triplets; the grid follows the beat and a note's first pick always plays), or `ENGINE_DEFAULT` (consult per-engine table). Set explicitly per voice when EDM/Space want different envelope behavior (e.g. `PLUCK` for a WSH bass on EDM, `BYPASS` for a STR drone on Space).
