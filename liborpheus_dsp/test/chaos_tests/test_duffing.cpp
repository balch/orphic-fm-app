// Duffing driven oscillator: divergence test
#include "test_harness.h"
#include "orpheus_unit_chaos.h"

static bool test_chaos_duffing_diverges() {
    printf("\n=== Test: Duffing engine 202 diverges ===\n");
    OrpheusEngine* engine = orpheus_engine_create(48000.0f);
    engine->voice_params[0].active.store(1);
    engine->voice_params[0].ever_triggered.store(1);
    engine->voice_params[0].engine_index.store(kChaosEngineDuffing);
    engine->voice_params[0].tune.store(60.0f);
    engine->voice_params[0].gate.store(1);
    engine->voice_params[0].harmonics.store(0.7f);  // strong drive
    ...
    printf("  Duffing amp: %.4f %s\n", amp, ok ? "OK" : "FAIL");
    orpheus_engine_destroy(engine);
    return ok;
}

bool run_chaos_duffing_tests() {
    int suite_pass = 0, suite_fail = 0;
    if (test_chaos_duffing_diverges()) suite_pass++; else suite_fail++;
