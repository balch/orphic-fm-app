...
// Extends with:
//   - Linkwitz-Riley 2nd-order crossover (treble/bass split)
...
// ─── Crossover filter state (one per channel, 2nd-order Butterworth cascade) ──
// We use a simple one-pole LP/HP pair for the crossover.
// A true Linkwitz-Riley 4th-order requires cascading two Butterworth 2nd-orders,
// but a single one-pole LP + complementary HP (input - LP) is sufficient for a
// Leslie crossover and avoids phase cancellation artifacts in the recombined sum.
...
struct OrpheusHorn {
        ...
        float smooth_coeff_val)
    {
        ...
        const float a = depth_param * 160.0f;   // slow LFO contribution
        const float b = depth_param * 16.0f;    // fast LFO contribution
        ...
        for (int i = 0; i < num_frames; ++i) {
            ...
            // ── Woofer LFO (bass rotor) — 3 phases at 120° offsets ──
            float wp_angle = woofer_phase * k2Pi;
            float fast_0   = std::sin(wp_angle);
            float fast_120 = std::sin(wp_angle + k2Pi * k1Over3);
            float fast_240 = std::sin(wp_angle + k2Pi * k2Over3);

            // ── Modulation offsets (samples) ──
            // MI mixes slow (horn) + fast (woofer) into each tap.
            // We keep same topology but use the split signals per rotor.
