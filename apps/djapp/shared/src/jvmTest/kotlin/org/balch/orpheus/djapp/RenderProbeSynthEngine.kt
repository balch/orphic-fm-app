...
/**
 * Fully inert [SynthEngine], here only so the real visualizations can be constructed in the
 * render harness — they take an engine in their constructor but never touch it while inactive,
 * which is exactly the state a headless render leaves them in.
 *
 * A fourth copy of this boilerplate, after `NoOpSynthEngine` (features/pulsar commonTest),
 * `TestSynthEngine` (apps/orpheus/shared) and `SongEndingStubSynthEngine`. None is reachable
 * from here: each lives in another module's test source set, and those do not travel.
 */
class RenderProbeSynthEngine : SynthEngine {
    override fun start() {}
    override fun stop() {}
