...
## Overview
Orphic-FM is modeled after vintage organismic drone synths -- instruments where simple oscillators are hierarchically grouped and cross-modulated until the sound takes on a life of its own. Eight voices pair into four duos, which group into two quads, with modulation layered at every level. Add a dual delay system that can self-oscillate, a plate reverb, stereo distortion, add in AI agent control and things get interesting fast.
...
`SynthController` is the central bus. Every control event carries an `origin` (`MIDI`, `UI`, `SEQUENCER`, `TIDAL`, `AI`, `EVO`) so the system knows who's driving a parameter and avoids conflicts. ViewModels observe `StateFlow` and conditionally skip engine writes based on origin.
...
| [Liquid](https://github.com/nichenqin/liquid) | Glassmorphism blur effects for Compose |
...
| [Koog Agents](https://github.com/koog-ai/koog-agents) | AI agent framework with Gemini integration |
| [Ktor](https://ktor.io/) | Kotlin async HTTP client |
| [KmLogging](https://github.com/nichenqin/KmLogging) | Kotlin Multiplatform structured logging by Lighthouse Games |
...
| [BuildKonfig](https://github.com/nichenqin/buildkonfig) | Cross-platform BuildConfig for KMP |
