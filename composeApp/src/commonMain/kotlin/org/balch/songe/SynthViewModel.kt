...
import org.balch.songe.features.preset.DronePresetRepository
...
class SynthViewModel(
    ...
    var hyperLfoMode by mutableStateOf(HyperLfoMode.OFF)
    ...
    var duoModSources by mutableStateOf(List(4) { ModSource.OFF })
        private set
    ...
    fun onHyperLfoModeChange(mode: HyperLfoMode) {
        hyperLfoMode = mode
    ...
    fun applyPreset(preset: DronePreset) {
        ...
        duoModSources = preset.duoModSources.mapIndexed { index, sourceStr ->
            try {
                ModSource.valueOf(sourceStr)
            } catch (e: Exception) {
                ModSource.OFF
            }
        }.take(4) + List(maxOf(0, 4 - preset.duoModSources.size)) { ModSource.OFF }
